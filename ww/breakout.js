...
let blockHeight = 10;
let blockColumns = 7; 
let blockRows = 8; //add more as game goes on
...
function update() {
    ...
    if (ball.y <= 0) { 
        ...
        context.fillText("Game Over: 按空白鍵重新開始", 50, 250);
        gameOver = true;
    }
...
function createBlocks() {
    ...
    for (let c = 0; c < blockColumns; c++) {
        for (let r = 0; r < blockRows; r++) {
            if ((c==0 && r>=1 && r<=4) ||(c==1 && r<=5) || (c==2 && r>=1 && r<=6) || (c==3 && r>=2 && r<=7) || (c==4 && r>=1 && r<=6) || (c==5 && r>=0 && r<=5) || (c==6 && r>=1 && r<=4)) {
                let block = {
